...
public class DefaultAbacusAKImplemetation extends DefaultAbacusABRow {
	...
	public Collection<Integer> generateARow() {
		Collection<Integer> rowA = new ArrayList<>();
		...
		for (Integer item : row1) {
			if (item >= 6) {
				row2.add(this.defaultAbacusABRowData.negativeSingleDigit(2, 3));
			} else {
		...
		for (Integer item : row2) {
			if (item < 0) {
				...
				row3.add(this.defaultAbacusABRowData.negativeSingleDigit(4, 6));
			}
		}
		rowA.addAll(row1);
		rowA.addAll(row2);
	...
	public Collection<Integer> generateBRow() {
		Collection<Integer> rowB = new ArrayList<>();
		...
		Collection<Integer> row4 = new ArrayList<>();
	...
	public Collection<Integer> generateCRow() {
		Collection<Integer> rowC = new ArrayList<>();
	...
	public Collection<Integer> generateDRow() {
		Collection<Integer> rowD = new ArrayList<>();
	...
	public Map<String, Collection<Integer>> getTables() {
		Map<String, Collection<Integer>> row = new HashMap<>();
